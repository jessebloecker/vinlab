...
class BSplineCore():
    """
    Given bspline degree and resolution, evaluate and store the basis vector and its
    ...
    Attributes:
        ...
        degree (int):  (1=linear, 2=quadratic, 3=cubic, etc.)
        basis_vectors (ndarray): (see function)
        blending_matrix (ndarray): (see function)
    """
    def __init__(self,res,degree):
        self.res = res
        self.degree = degree
        self.basis_vectors = self.eval_basis_vectors(res,degree)
        self.blending_matrix = self.get_blending_matrix(degree)
    ...
    def eval_basis_vectors(self,n,k,d=None):
        ...
        params:
            n (int): resolution - number of points per span 
            k (int): bspline degree (1=linear, 2=quadratic, 3=cubic, etc.)
            d (int): include all derivatives up to this degree.
        ...
        if d is None or d < 0:
            ...
            raise ValueError('d ({}) must be less than or equal to degree k ({})'.format(d,k))
        if k > max_k:
            raise ValueError('degree k ({}) must be less than or equal to {}'.format(k,max_k))
        ...
        b = B[0:d+1,:,0:k+1] #select the first d+1 layers and first degree+1 columns in each layer
        return b
    
    def get_blending_matrix(self, degree):
        """
        params:
            degree(int)
        returns:
            ndarray - blending matrix corresponding 'degree'
        """
        if degree==0:
            M = np.array([1.0])
        elif degree==1:
            ...
            raise ValueError('bspline degree \'{}\' is invalid or not supported - must an integer between 2 and 7 inclusive.'.format(degree))
        return M
    
    @classmethod
    def print_blending_matrix(cls,degree):
        """
        print out the blending matrix 'M' for the given bspline degree
        params:
            degree(int)
        """
        k = degree
        ...
        print('blending matrix for degree {} bspline: \n M = (1/({}!)) * \n{}\n'.format(k,k,M))
    

if __name__ == '__main__':
    np.set_printoptions(precision=4, suppress=True)
    BSplineCore.print_blending_matrix(0)
    BSplineCore.print_blending_matrix(1)
